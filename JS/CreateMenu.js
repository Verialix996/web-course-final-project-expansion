...
form.addEventListener('submit', function (e) {
    ...
    .then(data => {
        if (data.success) {
            ...
            showSuccess(data.user_name, data.load_level, data.id);
        } else {
...
function showSuccess(name, level, id) {
    const levelNames = { easy: 'קל', normal: 'רגיל', intense: 'אינטנסיבי' };
    const idBlock = id
        ? `<div style="background:rgba(0,0,0,0.08);border-radius:8px;padding:0.8rem 1.2rem;margin:0.8rem 0;display:inline-block">
               🔑 קוד הכניסה שלך (שמור אותו!): <strong style="font-size:1.6rem;letter-spacing:6px;color:var(--primary)">${id}</strong>
               <br><small style="color:#666">תזדקק לשם + קוד זה בכניסות הבאות</small>
           </div>`
        : `<div style="color:#e74c3c;margin:0.5rem 0;font-size:0.9rem">⚠️ השרת לא זמין – התפריט נשמר מקומית בלבד. קוד כניסה לא נוצר.</div>`;
